...
void WriteDataLin(bitstream *b, unsigned char ***image, int xmax, int ymax, int isColored)
{
    ...
    if (isColored)
    {
        for (int i = 0; i < xmax; i++)
        {
            ...
            { // Pour chaque pixels de l'image, donc 1 octet
                for (int k = 0; k < 3; k++)
                { // pas 4 car on veut pas ecrire sur le canal de translucense(alpha channel)
                    ...
                    bitpos++;
                    if (bitpos >= 8)
                    { // Tant qu'on a pas écrit l'octet en entier, on ne bouge pas!
                        bitpos = 0;
                        b->position++;
